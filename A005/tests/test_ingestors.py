...
# Fixture é uma função que é executada antes de todos os testes
@pytest.fixture # Decorador de fixture
...
@patch("ingestors.DataIngestor.__abstractmethods__", set()) # É um curativo, sobrescreve um método de uma classe
class TestIngestors:
    ...
    def test_load_checkpoint_no_checkpoint(self, data_ingestor_fixture): # Teste checkpoint desconhecido
        actual = data_ingestor_fixture._load_checkpoint()
        expected = datetime.date(2021, 6, 21) # Se eu não tiver checkpoint, esse é meu valor esperado
        assert actual == expected

    @patch("builtins.open", new_callable=mock_open, read_data="2021-06-25") # Substitui a função open e read
    def test_load_checkpoint_existing_checkpoint(self, mock, data_ingestor_fixture): # Teste para checkpoint existente // Fixture é o último argumento
        actual = data_ingestor_fixture._load_checkpoint()
    ...
    def test_write_checkpoint(self, mock_checkpoint_filename, mock_open_file, data_ingestor_fixture): # Primeiro patch, vai por último nos parâmetros
